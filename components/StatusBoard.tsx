            ...
            <div>
              ...
              <div className="space-y-6">
                {state.unlockedParties.map((isUnlocked, partyIndex) => {
                  ...
                  const partyHeroes = presetIds
                    .map(id => state.heroes.find(h => h.id === id))
                    .filter((h): h is any => !!h);
                  
                  // Fill empty slots
                  const displayParty = [...partyHeroes];
                  while (displayParty.length < 3) displayParty.push(null);

                  const isActive = state.activePartyIndex === partyIndex;
                  const isQuesting = displayParty.some(h => h && state.activeQuests.some(q => q.heroIds.includes(h.id)));

                  return (
                    <div key={partyIndex} className={`relative p-4 rounded-2xl border transition-all ${
                      isActive 
                        ? 'bg-indigo-900/10 border-indigo-500/30 shadow-[0_0_15px_rgba(79,70,229,0.1)]' 
                        : 'bg-slate-900/40 border-slate-800'
                    }`}>
                      <div className="flex justify-between items-center mb-3">
                        <div className="flex items-center gap-2">
                          <h3 className={`text-sm font-bold font-orbitron ${isActive ? 'text-indigo-300' : 'text-slate-400'}`}>
                            Party {partyIndex + 1}
                          </h3>
                          {isActive && (
                            <span className="text-[8px] bg-indigo-600 text-white px-2 py-0.5 rounded-full font-bold">
                              Selected
                            </span>
                          )}
                        </div>
                        {isQuesting && (
                          <span className="text-[9px] text-orange-400 font-bold flex items-center gap-1 animate-pulse">
                          ...
                          <div key={index} className="pointer-events-none">
                            {hero ? (
                              <HeroCard 
                                ...
                                index={index} 
                                isMainSlot
                                // Pass isLocked=true if this specific hero is questing to visually dim it
                                isLocked={state.activeQuests.some(q => q.heroIds.includes(hero.id))}
                              />
                            ) : (
                              <div className="w-full aspect-square bg-slate-900/50 rounded-xl border border-slate-800 flex items-center justify-center">
