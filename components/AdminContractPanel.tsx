...
const AdminContractPanel: React.FC = () => {
  ...
  const fetchHistory = async (contractAddress: string) => {
    ...
    try {
      ...
      const tokenTxUrl = `https://api-v2.basescan.org/api?module=account&action=tokentx&contractaddress=${CHH_TOKEN_ADDRESS}&address=${contractAddress}&page=1&offset=15&sort=desc&apikey=${apiKey}&chainid=8453`;
      ...
      const normalTxUrl = `https://api-v2.basescan.org/api?module=account&action=txlist&address=${contractAddress}&page=1&offset=15&sort=desc&apikey=${apiKey}&chainid=8453`;
