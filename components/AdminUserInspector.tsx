...
const AdminUserInspector: React.FC = () => {
  ...
  const handleForceArchive = async (quest: any) => {
    if (!confirm(`クエスト「${quest.quest_mining?.name || quest.quest_pid}」を強制的に完了履歴(Archive)に移動しますか？\n\n注意:\n・Activeテーブルから削除され、Completeテーブルに追加されます。\n・報酬トークンの自動付与は行われません（レコード移動のみ）。`)) return;
    ...
    try {
        ...
        const { error: insertError } = await supabase
            ...
            .insert({
                ...
                // party_id is not in the schema of quest_process_complete
                reward: totalReward,
                // created_at will be auto-generated by DB or we can pass if schema requires
            });
                                    ...
                                    <div className="text-right flex flex-col items-end gap-1 ml-2 shrink-0 self-start">
                                        ...
                                        <div className="flex gap-1">
                                          ...
                                          <button 
                                            ...
                                            title="Move to Completed Table (No Rewards)"
                                          >
                                            📂 ARCHIVE
                                          </button>
                                        </div>
